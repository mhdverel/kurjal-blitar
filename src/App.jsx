  ...
  return (
    <div className="app-shell">
      ...
      <div className="workspace">
        ...
        <MobileNav role={role} page={page} nav={nav} onNavigate={navigate} onMenu={() => setMenuOpen(true)} />
      </div>
      <dialog
...
function Sidebar({ role, page, nav, open, onNavigate, onClose }) {
  return <>
    ...
    <aside className={`sidebar ${open ? 'sidebar--open' : ''}`}>
      <div className="sidebar__head"><Brand /><button className="icon-button sidebar__close" type="button" aria-label="Tutup menu" onClick={onClose}><X size={20} /></button></div>
      <div className="sidebar__role">Ruang kerja · {role === 'ADMIN' ? 'Admin' : 'Driver'}</div>
      <nav aria-label="Navigasi utama">{nav.map(([key, label, Icon]) => <button type="button" key={key} className={page === key ? 'active' : ''} aria-current={page === key ? 'page' : undefined} onClick={() => onNavigate(key)}><Icon size={18} /><span>{label}</span></button>)}</nav>
...
function MobileNav({ role, page, nav, onNavigate, onMenu }) {
  const keys = role === 'ADMIN'
    ? ['dashboard', 'drivers', 'orders', 'settlements']
    : ['home', 'record', 'deliveryFee', 'history']
  const items = keys.map((key) => nav.find(([itemKey]) => itemKey === key)).filter(Boolean)
  const menuActive = !keys.includes(page)
  return <nav className="mobile-nav" aria-label="Navigasi bawah">{items.map(([key, label, Icon]) => <button type="button" key={key} className={`${page === key ? 'active' : ''} ${key === 'record' ? 'mobile-nav__record' : ''}`} aria-current={page === key ? 'page' : undefined} onClick={() => onNavigate(key)}><Icon size={19} /><span>{label}</span></button>)}<button type="button" className={menuActive ? 'active' : ''} aria-current={menuActive ? 'page' : undefined} onClick={onMenu}><Menu size={19} /><span>Menu</span></button></nav>
}
...
function LoginPage({ onLogin, onRegister, configurationError }) {
  ...
  return <section className="login-card"><div className="login-copy"><span className="eyebrow">Portal KURJAL</span><h1>Satu pintu untuk operasional Blitar.</h1><p>Admin dan driver masuk dari portal yang sama. Akses ditentukan otomatis dari profil akun.</p><div><BadgeCheck size={20} /><span><b>Routing aman sesuai role</b><small>Role dan status dibaca dari Firestore.</small></span></div></div><form className="login-form" onSubmit={submit}><span className="eyebrow">Selamat datang</span><h2>Masuk ke KURJAL</h2><p>Gunakan email dan password akun Anda.</p><label>Email<input type="email" autoComplete="email" inputMode="email" enterKeyHint="next" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="nama@email.com" required /></label><label>Password<input type="password" autoComplete="current-password" enterKeyHint="go" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} placeholder="Masukkan password" required /></label>{(error || configurationError) && <p className="form-error" role="alert">{error || configurationError}</p>}<button className="button button--primary button--full" disabled={submitting}>{submitting ? 'Memeriksa akun…' : 'Masuk'}</button><p className="register-link">Belum punya akun? <button type="button" className="text-button" onClick={onRegister}>Daftar sebagai Driver</button></p></form></section>
}
