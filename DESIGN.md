...
seo:
  title: "Airtable Design System for React — near-black ink, Haas Grotesk, and 26 components"
  metaDescription: "Airtable's design system as a DESIGN.md file. Near-black #181d26, Haas Grotesk, 28 colors, 26 components. For React, Next.js, and AI."
  highlights:
    - "Near-black primary — #181d26 is the brand CTA, not the link blue (#1b61c9) hiding in the CSS variables"
    - "Modest type weights — display headlines stay at 400–500; weight is delegated to size and signature surface cards"
    - "Signature surface cards — full-bleed coral, forest, navy, and cream blocks punctuate long-scroll pages every two or three screens"
    - "Pricing is its own dialect — Inter Display at weight 475 plus pill-shaped buttons that don't appear anywhere else"
    - "White hero, full stop — no gradient, no mesh, no atmospheric backdrop; whitespace and Haas do the framing"
  tags:
    - "Workflow & No-code"
    - "Productivity & SaaS"
  ...
  questions:
    - id: "primary-color"
      title: "What is Airtable's primary brand color?"
      answer: "Near-black #181d26. It's the same hex as the ink token because Airtable uses the same role for both primary button background and h1/h2 display type. Notably, this is NOT the #1b61c9 blue that appears as --theme_button-background-primary in the CSS — that misleadingly-named variable is actually the inline-link color, not the brand action. Treating link-blue as the primary is the most common mistake when reading Airtable's CSS."
    - id: "typography"
      title: "What typography does Airtable use, and what should I use if Haas isn't available?"
      answer: "Haas Grotesk and Haas Groot Disp — a custom display variant Airtable licenses. Haas Groot Disp covers display sizes 32–48px; Haas Grotesk covers everything 24px and below. The pricing surface uses a separate Inter Display stack at an unusual 475 weight, marking it as a deliberate sub-system. The closest open-source substitute for both is Inter Display — adjust line-height down by ~5% to match Haas's tighter cap height."
    - id: "modest-weights"
      title: "Why are Airtable's display headlines so modest in weight?"
      answer: "The system delegates emphasis to size and color contrast, not to weight. A 40px h1 sits at weight 400, not 700 — going bolder would read as 'another SaaS marketing template' instead of Airtable's quiet editorial confidence. The only true bold (600) in the whole system lives in legal CTAs, signaling 'this is a required system surface, not a designed brand surface.'"
    - id: "long-scroll-rhythm"
      title: "How does Airtable break up its long-scroll explainer pages?"
      answer: "With signature surface cards. Three distinct surfaces — dark coral (#aa2d00), forest green (#0a2e0e), and navy (#181d26) — punctuate the editorial body every two or three screens. Between them sit white-canvas bands with headlines and copy, demo-card grids on warm pastel surfaces (peach, mint, yellow, mustard), and cream callouts. The canvas always resets to white between signature surfaces — never two darks in a row."
    - id: "pricing-subsystem"
      title: "What's different about Airtable's pricing page?"
      answer: "It's a deliberate sub-system with its own typographic voice. The pricing surface uses Inter Display at weight 475 (a custom mid-weight between regular and medium) and pill-shaped buttons (9999px radius) that don't appear anywhere else in the system. The shape and font dialect signal 'this page is about commercial precision,' not editorial storytelling. Mixing the pricing dialect with the main marketing surface breaks the sub-system's voice."
    - id: "use-in-project"
      title: "Can I use this DESIGN.md to build my own workflow product?"
      answer: "Yes — feed it to Claude, Cursor, or GitHub Copilot. The agent will reproduce Airtable's restraint (near-black primary, modest type weights, signature surface cards) rather than a generic SaaS theme. You can also reference the tokens directly — every color, type style, radius, and spacing value is a quoted value ready to paste into Tailwind config, CSS variables, or your own component library."
